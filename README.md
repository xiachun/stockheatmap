...
- 申万行业权重不是 iFinD 直接返回的，而是程序将样本股映射到申万一级行业后自行汇总计算
...
LATEST_FORCE_REFRESH_MIN_INTERVAL_MS=1000
...
总览图中的行业权重来源：
- 先将样本指数成分股映射到申万一级行业
- 再按 `weightAnchorDate` 的样本股流通市值在行业内求和
- 最后用 `行业样本流通市值 / 全部样本流通市值` 作为行业权重

也就是说，程序使用的是“样本指数在申万一级行业维度上的市值分布”，不是 iFinD 直接返回的一套行业权重。
...
- `sampleSectorProxyReturnPct`
  样本行业代理口径收益率。计算方法是：
  `Σ(样本行业权重 × 申万一级行业指数收益率)`

- `sampleConstituentWeightedReturnPct`
  样本成分股口径收益率。计算方法是：
  `Σ(样本个股权重 × 样本个股收益率)`

- `sampleUniverse`
  当前样本空间说明，例如“中证A500成分股（按申万一级行业分类）”

其中：
- 样本行业权重 = 该行业内样本股流通市值之和 / 全部样本股流通市值之和
- 样本个股权重 = 单只样本股流通市值 / 全部样本股流通市值
- 两者通常不会相等，因为前者使用的是“样本行业权重 × 行业指数收益率”，后者使用的是“样本个股权重 × 个股收益率”
